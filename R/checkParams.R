checkParams <- function(p,checkPositiveDefinite=FALSE) {
  p$type <- checkType(p$type)
  if(!("matrix" %in% class(p$mu_z_t)) || any(is.na(p$mu_z_t))) {
    stop("p$mu_z_t not correspondingly set")
  }
  if(!("matrix" %in% class(p$Sigma_z_t)) || any(is.na(p$Sigma_z_t))) {
    stop("p$Sigma_z_t not correspondingly set")
  }
  ...
  nDimZ <- p$nDimZ
  if(nDimZ != ncol(p$mu_z_t) ||
    nDimZ != ncol(p$Sigma_z_t) ||
     nDimZ != nrow(p$Sigma_z_t) ||
     nDimZ != nrow(p$lambda_t)) {
    stop("problem with number of latent dimensions in p")
  }
  if(checkPositiveDefinite) {
    if(!is.positive.definite(p$Sigma_z_t)) {
      stop("p$Sigma_z_t is not positive definite")
    }
    if(!(p$type %in% c("ppca","fa"))) {
      # print(p$psi_t)
